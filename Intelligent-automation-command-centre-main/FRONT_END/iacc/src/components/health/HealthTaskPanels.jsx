...
import { DOCTORS, HOSPITALS, DHOS } from '../../data/healthData';
...
// ════════════════════════════════════════════════════════════════
export const TaskAssignPanel = ({ user, role, department = 'HEALTH', accentColor = '#3b82f6' }) => {
    ...
    const memberList = () => {
        ...
        if (role === 'CMO') {
            const extraHMs = localUsers.filter(u => u.role === 'HOSPITAL_WARDEN');
            return [
                ...HOSPITALS.map(h => ({ id: h.id, name: h.name, role: 'HOSPITAL_WARDEN', sub: h.type })),
                ...extraHMs,
                ...DHOS.filter(d => d.zone).map(d => ({ id: d.id, name: d.name, role: 'DHO', sub: d.zone })),
            ];
        }
        if (role === 'HOSPITAL_WARDEN') {
            const extraTeachers = localUsers.filter(u => u.role === 'DOCTOR');
            return [...DOCTORS.map(t => ({ id: t.id, name: t.name, role: 'DOCTOR', sub: t.dept })), ...extraTeachers];
        }
        return [];
    };
    ...
    const incoming = tasks.filter(t => {
        ...
        if (role === 'HOSPITAL_WARDEN') {
            return (
                ...
                t.reassignedToRole === 'HOSPITAL_WARDEN' ||   // role-based (demo: any HM can see)
                (t.assignedToRole === 'HOSPITAL_WARDEN' && !t.fromCollector) // HM assigned by CEO
            ) && t.assignedByRole !== 'HOSPITAL_WARDEN';      // exclude self-created
        }
        return false;
    });
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                    ...
                                    <button
                                        onClick={() => {
                                            ...
                                            if (v) onSubmit({ id: v, name: v, role: 'HOSPITAL_WARDEN', sub: 'Custom Member' });
                                        }}
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    <button
                        onClick={() => {
                            ...
                            if (v) onSelect({ id: v, name: v, role: 'HOSPITAL_WARDEN', sub: 'Custom Member' });
                        }}
                        style={{ padding: '0.5rem 1rem', background: accentColor, color: '#fff', border: 'none', borderRadius: '0.5rem', fontWeight: 600, cursor: 'pointer', fontSize: '0.8rem' }}
                    >Delegate</button>
                </div>
...
// ════════════════════════════════════════════════════════════════
export const TaskStatusPanel = ({ user, role, department = 'HEALTH', accentColor = '#3b82f6' }) => {
    const { tasks } = useDeptTaskStore();
    const username = user?.username || '';
    const [filter, setFilter] = useState('ALL');

    // CEO sees all HEALTH tasks; HM sees tasks they created or were assigned to them
    const myTasks = role === 'CMO'
...
export const MemberTasksPanel = ({ user, role, memberName, accentColor = '#8b5cf6' }) => {
    ...
    const myTasks = tasks.filter(t => {
        ...
        if (memberRole === 'HOSPITAL_WARDEN' && t.reassignedToRole === 'HOSPITAL_WARDEN') return true;
        if (memberRole === 'DOCTOR' && (
            t.reassignedToRole === 'DOCTOR' ||
            t.assignedToRole === 'DOCTOR'
        )) return true;
        return false;
    });
